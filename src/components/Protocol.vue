<template>
  <v-container>
    <!-- 标题 -->
    <v-row>
      <p class="title font-weight-bold mb-3">拓扑配置及校验</p>
    </v-row>
    ...
    <v-row>
      ...
      <v-col sm="2">
        <v-btn elevation="4" color="primary" :loading="loader_config" @click="config_script">文本配置拓扑</v-btn>
      </v-col>
      <v-col sm="5">
        <v-textarea v-model="protocol_content" outlined placeholder="这里是配置或校验文件"></v-textarea>
      </v-col>
      <!-- 拓扑校验 -->
      <v-col sm="2">
        <v-btn elevation="4" color="primary" :loading="loader_verify" @click="verify">文本校验拓扑</v-btn>
      </v-col>
    </v-row>
    ...
    <!-- 控制台 -->
    <v-row>
      <p class="title font-weight-bold mb-3">控制台</p>
    </v-row>
    <v-row>
      <!-- 设备选择 -->
      <v-col cols="8" sm="2">
        <v-select v-model="dev_selected" :items="dev_list" label="设备选择" outlined></v-select>
      </v-col>
      <!-- ping -->
      <v-col cols="8" sm="2">
        <v-text-field v-model="ping_ip" label="ip" required outlined></v-text-field>
      </v-col>
      <v-col sm="1">
        <v-btn elevation="4" color="primary" :loading="loader_ping" @click="ping">ping</v-btn>
      </v-col>
    </v-row>
    <!-- 输入控制台 -->
    <v-row>
  ...
  methods: {
    ...
    verify() {
      if (!this.protocol_content) {
        ...
        axios({
          ...
          console.log(res)
          if (res.data.state) {
            this.showMessage('mdi-checkbox-marked-circle', 'test success', 'success')
            this.setLoader('verify', false)
          } else {
            this.showMessage('mdi-cancel', 'test fail', 'error')
            this.setLoader('verify', false)
          }
        }).catch(err => {
          console.log(err)
    ...
    ping() {
      ...
      axios({
        ...
        if (res.data.state) {
          this.showMessage('mdi-checkbox-marked-circle', 'ping success', 'success')
          this.setLoader('ping', false)
        } else {
          this.showMessage('mdi-cancel', 'ping fail', 'error')
          this.setLoader('ping', false)
        }
        // 输出信息到控制台
        this.outmsg = res.data.msg
      }).catch(err => {
        this.showMessage('mdi-minus-circle', err, 'warning')
        this.setLoader('ping', false)
      })
    },
    // 向设备发送命令
    send_command() {
      ...
      axios({
        ...
        if (res.data.state) {
          // this.showMessage('mdi-checkbox-marked-circle', 'send success', 'success')
          this.setLoader('command', false)
        } else {
          this.showMessage('mdi-cancel', 'send command fail', 'error')
          this.setLoader('command', false)
        }
        // 输出信息到控制台
        this.outmsg = res.data.msg
